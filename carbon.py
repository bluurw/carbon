    ...
    async def toFoward(self, source, destination, filter=[], limit=None):
        ...
        try:
            ...
            async for message in client.iter_messages(source_entity, reverse=True, limit=limit):
                print(f'\033[1;33m[INFO]\033[0m[{message.date}] METADATA: {message.id} {message.entities}')
                system.load_log(f'[INFO][{message.date}] msg.ID: {message.id} group.ID: {message.chat_id} {message.out} {message.entities}\n')
                try:
                    await asyncio.sleep(0.85) # wait
                    
                    if len(filter) == 0:
                        print(f'\033[1;33m[INFO]\033[0m None Type: {message.id}\n')
                        system.load_log(f'[INFO] {message.id}\n')
                        ...
                        elif 'audios' in filter and message.audio or message.voice:
                            print(f'\033[1;33m[INFO]\033[0m audio {message.id}\n')
                            system.load_log(f'[INFO] audio {message.id}\n')
                            await client.send_file(destination, message.media, caption=message.text)

                        elif 'stickers' in filter and message.sticker:
                            print(f'\033[1;33m[INFO]\033[0m sticker {message.id}\n')
                            system.load_log(f'[INFO] sticker {message.id}\n')
                            await client.send_file(destination, message.media, caption=message.text)
        ...
        try:
            ...
            async for message in client.iter_messages(source_entity, reverse=True, limit=limit):
                ...
                try:
                    ...
                    if len(filter) == 0:
                        if message.media:
                            os.makedirs(f'{path_save}/medias/', exist_ok=True) # if exists
                            file_path = await message.download_media(file=f'{path_save}/medias/')
                            print(f'\033[1;33m[INFO]\033[0m media saved {message.id} {file_path}\n')
                            system.load_log(f'[INFO] media saved {message.id} {file_path}\n')
                        ...
                        if 'images' in filter and message.photo:
                            ...
                            os.makedirs(f'{path_save}/stickers/', exist_ok=True)
                            file_path = await message.download_media(file=f'{path_save}/stickers/')
                            print(f'\033[1;33m[INFO]\033[0m sticker saved {message.id} {file_path}\n')
                            system.load_log(f'[INFO] sticker saved {message.id} {file_path}\n')
